...
export default class Model {
  ...
  // }
  addQuestion(title, summary, text, tags, username) {
    ...
    for (let i = 0; i < tags.length; i++) 
    {
      ...
      if (existingTag){
        if (listOfTags.includes(existingTag.tid)) {
          continue;
        }
        listOfTags.push(existingTag);
      } 
      else {
  ...
  getAnsIdsByQid(qid) {
    return this.data.questions.find(ques => ques.qid === qid).ansIds;
  }
}
